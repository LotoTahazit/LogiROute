...
class OsrmDirectionsService {
  ...
  /// [avoidHighways], [avoidTolls], [avoidFerries] — пока не поддерживаются OSRM-сервером,
  /// но можно реализовать через локальную фильтрацию, если нужно.
  ...
  /// [optimize] — оптимизировать порядок точек (через OSRM Trip API)
  Future<RouteInfo?> getRouteWithWaypoints({
    ...
    try {
      // Формируем строку всех координат
      final coords = [
        '$originLng,$originLat',
...
class RouteInfo {
  ...
  final int durationInTraffic; // секунды с учётом трафика (у OSRM ≈ duration)
  final String polyline;
  final List<int>? waypointOrder; // оптимизированный порядок точек
  ...
  /// Время с "трафиком" — берём максимум между рассчитанным и скоростью 90 км/ч
