...
class SummaryService {
    ...
    try {
      ...
      await _firestore.runTransaction((transaction) async {
        ...
        if (!summary.exists) {
          ...
          final byStatus = (data['byStatus'] as Map?)?.map(
                (k, v) => MapEntry(k as String, (v as num).toInt()),
              ) ??
              <String, int>{};
          final byDriver = (data['byDriver'] as Map?)?.map(
                (k, v) => MapEntry(k as String, (v as num).toInt()),
              ) ??
              <String, int>{};
  ...
  Future<void> updateDeliverySummary(DeliveryPoint point) async {
    try {
      // Используем дату из точки доставки: openingTime → updatedAt → now
      final date = point.openingTime ?? point.updatedAt ?? DateTime.now();
      final dateKey = _getDateKey(date);
      final summaryRef =
