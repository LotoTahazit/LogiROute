...
/// Safely converts a Firestore map with num values to Map\<String, int\>.
/// On web, Firestore returns JavaScript numbers as `num`, not `int`.
...
class DailySummary {
  ...
  factory DailySummary.fromMap(Map<String, dynamic> map, String id) {
    return DailySummary(
      date: id,
      totalInvoices: ((map['totalInvoices'] ?? 0) as num).toInt(),
      totalAmount: (map['totalAmount'] is num)
          ? (map['totalAmount'] as num).toDouble()
          : 0.0,
      byStatus: _toIntMap(map['byStatus']),
      byDriver: _toIntMap(map['byDriver']),
...
class DeliverySummary {
  ...
  factory DeliverySummary.fromMap(Map<String, dynamic> map, String id) {
    return DeliverySummary(
      date: id,
      totalPoints: ((map['totalPoints'] ?? 0) as num).toInt(),
      pending: ((map['pending'] ?? 0) as num).toInt(),
      assigned: ((map['assigned'] ?? 0) as num).toInt(),
      inProgress: ((map['inProgress'] ?? 0) as num).toInt(),
      completed: ((map['completed'] ?? 0) as num).toInt(),
      cancelled: ((map['cancelled'] ?? 0) as num).toInt(),
      byDriver: _toIntMap(map['byDriver']),
      lastUpdated: map['lastUpdated'] != null
          ? (map['lastUpdated'] as Timestamp).toDate()
          : DateTime.now(),
