...
| פלטפורמה | Web (Flutter), Android |
| ארכיטקטורה | SaaS — Cloud-based (Google Firebase) |
| אזור אחסון | nam5 (United States) — Google Cloud multi-region |
...
| איסור שינוי מסמך שעבר הנפקה | ✅ | docNumber, issuedAt, immutableSnapshotHash — immutable ב-Rules |
| יומן ביקורת | ✅ | append-only audit subcollection (cross-module) |
| שמירה 7 שנים | ✅ | retention policy + automated checks |
| גיבוי תקופתי | ✅ | PITR (7 ימים) + יומי (14 ימים) + רבעוני לתאימות |
...
| דרישה | סטטוס | מימוש |
|--------|--------|--------|
| חתימת שלמות (hash) | ✅ | SHA-256 על 5 שדות ליבה (SnapshotHash.compute, JSON canonical) |
...
| RBAC | ✅ | 7 roles (super_admin, owner, admin, accountant, dispatcher, driver, warehouse_keeper) |
...
| Data residency | ✅ | nam5 (United States) — Google Cloud multi-region |
